...
class AnaliseClienteMgKafkaProducerTest {
	...
	@BeforeEach
	void setUp() {
		KafkaAppProperties properties = new KafkaAppProperties(true, TOPICO);
	...
	void deveEnviarMensagemNoTopicoConfigurado() {
		...
		assertThat(mensagem.clienteId()).isEqualTo(7L);
		assertThat(mensagem.documento()).isEqualTo("12345678901");
		assertThat(mensagem.nome()).isEqualTo("Ana Costa");
		assertThat(mensagem.numerosInstalacao()).containsExactly("INST-001", "INST-002");
		assertThat(mensagem.publicadoEm()).isNotNull();
	}
